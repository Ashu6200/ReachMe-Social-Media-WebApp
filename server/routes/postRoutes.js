...
postRouter.post("/user/post", verifyToken, async (req, res) => {
    try {
...
//update user post
postRouter.put("/update/post/:id", verifyToken, async (req, res) => {
    try {
...
//Dislike
postRouter.put("/:id/dislike", verifyToken, async (req, res) => {
    try {
        const post = await Post.findById(req.params.id);
        if (!post.dislike.includes(req.user.id)) {
            if (post.like.includes(req.user.id)) {
...
//Comment 
postRouter.put("/comment/post", verifyToken, async (req, res) => {
    // try {
